...
try {
  ...
  app.get("/admin/profile/:username", function(req, res) {
   ...
   <path d="M15 13l2 2"></path>
          ...
          if (result.account.dataint.badges == "beta") {
   ...
   <path d="M10 16h2a2 2 0 1 0 0 -4h-2h2a2 2 0 1 0 0 -4h-2v8z"></path>
          ...
          } else if (result.account.dataint.badges == "beta2") {
   ...
   <path d="M10 16h2a2 2 0 1 0 0 -4h-2h2a2 2 0 1 0 0 -4h-2v8z"></path>
          ...
          } else if (result.account.dataint.badges == "beta3") {
   ...
   <path d="M10 16h2a2 2 0 1 0 0 -4h-2h2a2 2 0 1 0 0 -4h-2v8z"></path>
      ...
      <h3 class="card-title" style="margin: 0; padding-left: 0.5rem;"><a href="/game/profile/${result.account.playername}">${result.account.playername}</a></h3>
      </div>`
          } else if (result.account.op == 2) {
            tr.rank = `<div style="line-height: inherit; display: flex; align-items: center;">
      <span class="badge" style="font-size:0.9em; vertical-align: middle;background:#E87676">Admin</span>
      <h3 class="card-title" style="margin: 0; padding-left: 0.5rem;"><a href="/game/profile/${result.account.playername}">${result.account.playername}</a></h3>
      </div>`
          } else if (result.account.op == 1) {
            tr.rank = `<div style="line-height: inherit; display: flex; align-items: center;">
      <span class="badge" style="font-size:0.9em; vertical-align: middle;background:#C98FDE">Moderator</span>
      <h3 class="card-title" style="margin: 0; padding-left: 0.5rem;"><a href="/game/profile/${result.account.playername}">${result.account.playername}</a></h3>
      </div>`
          } else if (result.game.rank == 3) {
            tr.rank = `<div style="line-height: inherit; display: flex; align-items: center;">
      ...
      <h3 class="card-title" style="margin: 0; padding-left: 0.5rem;"><a href="/game/profile/${result.account.playername}">${result.account.playername}</a></h3>
            ...
            tr.rank = `<h3 class="card-title" style="margin: 0; padding-left: 0.5rem;"><a href="/game/profile/${result.account.playername}">${result.account.playername}</a></h3>`
          }
  ...
  io.sockets.on('connection', function(socket) {
    ...
    socket.on('grabStatics', function(id, tx) {
      ...
      dbc.reqstat(id)
        .then(result => {
          allowed = result;
          if ((!allowed || allowed.account.op <= 1) && (offline == true) && (tx == "0")) {
            onl = 0
            allowed = reason
          }
    ...
    socket.on("globalclear", () => {
      //activecon.push(pfxm)
      dbc.clearChat("__global__")
        .then(result => { })
    });

    socket.on("globalsendbuffer", (contents, patch, name, rank, op, level) => {
      tack = 0
      if (op == 3) {
        mname = "#EB3838"
      } else if (op == 2) {
      ...
      xm = `<idx prop="CLIENT" sid=${patch}>[<cbox style='color:${mlvl};'>${tyamp}</cbox>] <cbox style='color:${mname};'>${name}</cbox>: ${contents}</idx>`
      socket.emit("delocalizeGlobalsend", xm, patch, tack);
      socket.broadcast.emit("recieveGlobalsend", xm);
      //activecon.push(pfxm)
      dbc.postChat(xm, "__global__")
        .then(result => { })
    });
    ...
    socket.on("postAvatar", (id, ide) => {
      ...
      if (ide != "none") {
        aximg.postimg(ide, id)
          .then(ilink => {
            if (!ilink) {
              socket.emit("postAvatarResult", "error");
              return false;
            }
  ...
  <span class="badge" style="font-size:0.9em; vertical-align: middle;background:#EB3838">Developer</span>
  <h3 class="card-title" style="margin: 0; padding-left: 0.5rem;"><a href="/game/profile/${user.playername}">${user.playername}</a></h3>
</div>`
            } else if (user.op == 2) {
              user.playername = `<div style="line-height: inherit; display: flex; align-items: center;">
  <span class="badge" style="font-size:0.9em; vertical-align: middle;background:#E87676">Admin</span>
  ...
  <h3 class="card-title" style="margin: 0; padding-left: 0.5rem;"><a href="/game/profile/${user.playername}">${user.playername}</a></h3>
              ...
              user.playername = `<h3 class="card-title" style="margin: 0; padding-left: 0.5rem;"><a href="/game/profile/${user.playername}">${user.playername}</a></h3>`
            }
    ...
    //                                                                  //
    // // ---------------------------------------------------------- // //
    //             © Ninjam Studios 2023 - Published by IXM             //
    // // ---------------------------------------------------------- // //


    /*
    Note to developers: Code here is nested within the socket runtime and therefore is entirely
    asynchronous to meet clock runtime requirements. Game data is stored in /gamedata. Streamed
    content is stored in /gamedata/stream. The game client references /public/game/index.ejs and
    public/game/resources for all local references.

    DO NOT DOUBLE CALL SOCKETS.
    */

    socket.on("cacheGame", (location, callback) => {
      try {
        const data = yaml.load(fs.readFileSync('gamedata/stream'+location, 'utf8'));
        callback({
          status: "ok",
          data: data
        });
